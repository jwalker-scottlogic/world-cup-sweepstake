...
const PlayerTable = ({ teams, rows, onHover }) => {
  const isTeamEliminated = (team) => teams[team].isEliminated;

  const teamClassNames = (team, allTeams) =>
    classnames({
  ...
  const areAllPlayerTeamsEliminated = ({ goals, outcomes }) =>
  ...
  const renderRow = (row, rank) => {
    ...
    return (
      <tr key={row.name} className={classNames}>
        <td>{rank}</td>
        <td>{row.name}</td>
