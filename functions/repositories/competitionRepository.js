...
async function fetchWithAuthHeader(url) {
  ...
  if (!response.ok) {
    throw new Error(
      `Unexpected response fetching fixtures '${response.statusText}'`
    );
  }
...
async function getFixtures() {
  ...
  const minutesSinceLastLookup = lastApiLookup
    ? DateTime.utc().diff(DateTime.fromISO(lastApiLookup), "minutes").toObject()
        .minutes
    : null;

  if (!minutesSinceLastLookup || minutesSinceLastLookup > 1) {
    console.log(
      "1 Minute has passed since last API lookup, updating fixtures."
    );

    const fixtureResponse = await fetchWithAuthHeader(
      `${URL_CONSTANTS.BASE}/${URL_CONSTANTS.COMPETITION_CODE}/${URL_CONSTANTS.FIXTURES}`
    );
    const fixtures = fixtureResponse.matches.map(
...
async function getTeams() {
  ...
  if (teams.length === 0) {
    ...
    const apiTeams = await fetchWithAuthHeader(
      `${URL_CONSTANTS.BASE}/${URL_CONSTANTS.COMPETITION_CODE}/${URL_CONSTANTS.TEAMS}`
    );
